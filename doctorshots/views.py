...
def login(request):
    try:
        ...
        request.session['logueado'] = [
            q.cedula, q.usuario, q.clave, q.Rol, q.nombres]
        return render(request, 'doctorshots/index.html')
...
def eliminarEmpleado(request,id):
        ...
        if ses and (ses[3] == "2" or ses[3] == "1" ):
            ...
            return HttpResponse("Usted no tiene permiso para eliminar aprendices ")        
...
def editarEmpleado(request, id):
    ...
    if ses and (ses[3] == "2" or ses[3] == "1" ):
            ...
            return HttpResponse("Usted no tiene permiso para editar aprendices ")        


def actualizarEmpleado(request):
        ses = request.session.get('logueado', False)
        if (ses[3] == "2" or ses[3] == "1" ):
            try:
                id= request.POST['id']
                q = Usuarios.objects.get(pk=id)
